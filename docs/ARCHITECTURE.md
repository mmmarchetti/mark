...
- **Optional bounded 1-hop handoff** (`MARK_ROUTER_HANDOFF_ENABLED`, default OFF): if the first completion's tool calls all fall outside the current specialist's set, rebuild the context for the owning specialist and re-run the first completion **once** (cheap — only the cached-prefix tail recomputes).
